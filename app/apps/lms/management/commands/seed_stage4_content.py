...
CLOTHS = [
    ...
    ('Губки', [
        ('text',
         '<h3>🧽 Губки — с них начинается мойка</h3>'
         '<p>Сначала поверхность <strong>моем губкой</strong>: чистящее средство наносим '
         '<strong>на губку</strong>, а не на поверхность. А уже после губки доводим салфеткой.</p>'),
        ('image', 'Загрузите фото: корейская двусторонняя губка (жёлтая + зелёный абразив)'),
        ('text',
         '<h3>🟡 Корейская губка</h3>'
         '<p>Двусторонняя: <strong>мягкая поролоновая</strong> сторона + <strong>жёсткая '
         'зелёная абразивная</strong>.</p>'
         + li('Мягкой стороной — деликатно, без царапин',
              'Жёсткой (зелёной) — стойкий налёт и грязь')
         + note('<strong>Почему «корейская»:</strong> держит пену лучше обычной, быстрее сохнет '
                'и не царапает.')),
        ('text',
         '<h3>⛔ Жёсткой стороной НЕЛЬЗЯ</h3>'
         + warn('<strong>Поцарапает:</strong> стекло без царапин, хрусталь, тефлоновые '
                'сковородки, керамические плиты.')),
        ('text',
         '<h3>🧭 Где используем в клининге</h3>'
         + li('<strong>Мойка, раковина:</strong> чистит нержавейку до блеска, жёсткой стороной '
              'снимает налёт',
              '<strong>Решётка плиты:</strong> идеально для газовых решёток вместе с Антижиром',
              '<strong>Швы кафеля:</strong> в ванной чистить швы между плиткой',
              '<strong>Батареи, двери:</strong> убрать засохшую краску и пятна')),
        ('image', 'Загрузите фото: металлизированная губка (синяя/красная, абразивная сетка)'),
        ('text',
         '<h3>🔵 Металлизированная губка (фибра)</h3>'
         '<p>Поролон в <strong>абразивной металлизированной сетке</strong>. Для жёсткого '
         'оттирания там, где обычная губка не берёт.</p>'
         + li('Пригоревший нагар и жир', 'Нержавейка, кастрюли, противни, решётки плиты',
              'Стойкий налёт')
         + warn('Это абразив — <strong>осторожно</strong>: не тереть стекло, эмаль, тефлон, '
                'глянец и пластик (поцарапает). Сначала тест на незаметном участке.')
         + note('Жёстче поролоновой губки, но мягче металлической мочалки. Работать смоченной.')),
        ('image', 'Загрузите фото: цветные губки Vileda PUR Active (синяя/красная/жёлтая/зелёная)'),
        ('text',
         '<h3>🎨 Цветовой код губок</h3>'
         + li(chip('#3b82f6', '<strong>Синяя</strong> — посуда'),
              chip('#ef4444', '<strong>Красная</strong> — раковина / плита'),
              chip('#eab308', '<strong>Жёлтая</strong> — пыль'))
         + note('Цвет помогает <strong>не путать</strong> губки между зонами.')),
        ('text',
         '<h3>📋 3 правила для клинеров</h3>'
         + li('<strong>Раздельно:</strong> губка для посуды — отдельно, для раковины/плиты — '
              'отдельно, для туалета — отдельно. Не мешать!',
              '<strong>Цветовой код:</strong> синяя=посуда, красная=раковина/плита, жёлтая=пыль',
              '<strong>Менять каждую неделю:</strong> губка копит бактерии. Появился запах — '
              'сразу выбросить.')),
        ('text',
         '<h3>💡 Лайфхак после ремонта</h3>'
         + warn('Цементную пыль и клей <strong>не три сухой губкой</strong> — поцарапаешь. '
                'Сначала намочи водой, потом мягкой стороной.')),
    ]),
]
...
QUIZ = [
    ...
    ('Чем нельзя тереть жёсткой стороной губки?', [
        ('Стекло, хрусталь, тефлон, керамические плиты — поцарапает', True),
        ('Газовую решётку плиты', False),
        ('Нержавеющую раковину', False),
    ]),
    ('Как часто менять губку?', [
        ('Каждую неделю, а при запахе — сразу выбросить', True),
        ('Раз в год', False),
        ('Никогда', False),
    ]),
    ('Куда наносят чистящее средство при мойке губкой?', [
        ('На губку', True),
        ('Сразу много на поверхность', False),
        ('В воздух', False),
    ]),
]
