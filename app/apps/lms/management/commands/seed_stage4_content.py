...
CLOTHS = [
    ...
    ('Салфетка для пола', [
        ('image', ('lessons/images/microfiber-floor.jpg',
                   'Большая микрофибровая тряпка для пола (UrbanClean)')),
        ('text',
         '<h3>🧹 Что это</h3>'
         '<p>Большая махровая микрофибра — <strong>только для пола</strong>. Больше обычной '
         'салфетки, хорошо впитывает воду и собирает грязь с пола.</p>'
         + warn('<strong>Только для пола!</strong> Не использовать на мебели, поверхностях и '
                'технике — пол самая грязная зона.')),
        ('text',
         '<h3>🧭 Как пользоваться</h3>'
         + li('Намочить и <strong>хорошо отжать</strong> — пол моют влажной тряпкой, не мокрой',
              'Можно вручную или надеть на швабру/флаундер',
              'Двигаться от дальнего угла к выходу',
              'Менять воду, когда она грязная',
              'Ламинат и паркет — почти насухо (вода вредит стыкам)')),
        ('text',
         '<h3>🧺 Уход</h3>'
         + li('Стирать <strong>отдельно</strong> от других салфеток — самая грязная',
              'Без кондиционера, умеренная температура',
              'Хорошо просушивать, чтобы не было запаха')),
        ('text',
         ok('Правило: <strong>тряпка для пола = только пол</strong>. Для мебели и поверхностей — '
            'свои салфетки.')),
    ]),
]
...
QUIZ = [
    ...
    ('Для чего салфетка для пола?', [
        ('Только для пола — самая грязная зона', True),
        ('Для мебели и техники', False),
        ('Для финишной полировки стекла', False),
    ]),
    ('Какой должна быть тряпка для пола при мытье?', [
        ('Влажной, хорошо отжатой', True),
        ('Максимально мокрой, с лужами', False),
